-- ========================================
-- REDZONE - ADMIN FIX & PERMISSIONS
-- ========================================
-- Ce script corrige les permissions admin et crée les tables manquantes
...
-- Supprimer TOUTES les politiques existantes pour site_settings (approche dynamique)
DO $$ 
DECLARE
  pol RECORD;
BEGIN
...
-- Policy : Seuls les admins peuvent lire/modifier
...
-- Alternative : Politique basée sur l'email (si vous préférez)
-- Remplacez 'votre-email@exemple.com' par votre email admin
-- CREATE POLICY "Super admin by email"
--   ON vehicules FOR ALL
--   USING (
--     EXISTS (
--       SELECT 1 FROM profiles 
--       WHERE id = auth.uid() 
--       AND (role = 'admin' OR email = 'votre-email@exemple.com')
--     )
--   )
--   WITH CHECK (
--     EXISTS (
...
-- ========================================
-- 5. CRÉER UN ADMIN (INSTRUCTIONS)
-- ========================================

-- Pour créer un admin, exécutez cette commande après avoir créé votre compte :
-- 
-- UPDATE profiles 
-- SET role = 'admin' 
-- WHERE email = 'votre-email@exemple.com';
--
-- OU créez directement un admin via Supabase Dashboard > Authentication > Add User
-- Puis exécutez la commande UPDATE ci-dessus
...
-- ========================================
-- 7. AJOUT DES COLONNES MANQUANTES (cv_fiscaux, car_pass_url, is_manual_model)
-- ========================================
